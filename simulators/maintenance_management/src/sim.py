...
class Env(gym.Env):
    def __init__(self):
        ...
        action_space = {"repairers_capacity" : {"low": 1, "high": 100}, #3
    ...
    def operate_machine(self, env, repairers, spares, downtime_cost_hourly_machine, time_to_failure_min, time_to_failure_max, hours_to_repair_min, hours_to_repair_max):
        global cost

        while True:
            yield env.timeout(self.time_to_failure(time_to_failure_min, time_to_failure_max))
            t_broken = env.now
            #print(f'{env.now} machine broke')
            env.process(self.repair_machine(env, repairers, spares, hours_to_repair_min, hours_to_repair_max))
            yield spares.get(1)  # wait for 1 spare
            t_replaced = env.now
            #print(f'{env.now} machine replaced')
            cost += downtime_cost_hourly_machine * (t_replaced - t_broken)

    def repair_machine(self, env, repairers, spares, hours_to_repair_min, hours_to_repair_max):
        with repairers.request() as request:
            yield request
            yield env.timeout(self.generate_repair_time(hours_to_repair_min, hours_to_repair_max))
            yield spares.put(1)
    ...
    def factory_run(self, env, repairers, spares,
                      machines = 50,
        ...
        for i in range(machines):
            env.process(self.operate_machine(env, repairers, spares, downtime_cost_hourly_machine, time_to_failure_min, time_to_failure_max, hours_to_repair_min, hours_to_repair_max))

        while True:
            cost += repairer_hourly_rate * machine_operating_hours * repairers.capacity + spare_cost * spares.capacity
    ...
    def reset(self):
        ...
        self.obs = {"machines": self.machines,
    ...
    def step(self, action):
        ...
        simpyenv.process(self.factory_run(simpyenv, repairers, spares,
