...
class Teacher(SkillTeacher):
                                           ...
                                           **
                                             *
                                              *
                                               *
        """
        # In WASM we cannot pass dynamic JSON, so we pass it as string and
        # parse it here
        transformed_obs = json.loads(transformed_obs)
        action = json.loads(action)

        counter = transformed_obs[self.sensor_name][0]
    ...
    async def compute_success_criteria(self, transformed_obs, action) -> bool:
        ...
        return bool(transformed_obs[self.sensor_name][0] >= 10)
