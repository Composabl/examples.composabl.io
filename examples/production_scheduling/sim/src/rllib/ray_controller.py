...
from simulation.equipment import EquipmentNames
...
class RLLibController(Controller):
    ...
    def get_state(self) -> Dict[str, Any]:
        return self.bakery_state.get_state()
    ...
    def get_mask(self) -> array :
        ...
        #If both mixers are full, do not mix
        ...
        #If ovens are full, do not bake from mixer
        if(self.bakery_state.oven_1.current_recipe.name.value > 0 and self.bakery_state.oven_2.current_recipe.name.value > 0 and self.bakery_state.oven_3.current_recipe.name.value > 0):
            masking_array[10] = 0
            masking_array[12] = 0
            masking_array[14] = 0
            masking_array[11] = 0
            ...
            #If mixer 1 has a receipe in it and there is time remaining, do not bake
            if (self.bakery_state.mixer_1.current_recipe.name.value != 0 and self.bakery_state.mixer_1.time_remaining > 0) or (self.bakery_state.mixer_1.current_recipe.name.value == 0 and self.bakery_state.mixer_1.time_remaining == 0):
                masking_array[10] = 0
                masking_array[12] = 0
                masking_array[14] = 0

            #If mixer 2 has a receipe in it and there is time remaining or it is empty, do not bake
            if (self.bakery_state.mixer_2.current_recipe.name.value != 0 and self.bakery_state.mixer_2.time_remaining > 0) or (self.bakery_state.mixer_2.current_recipe.name.value == 0 and self.bakery_state.mixer_2.time_remaining == 0):
                masking_array[11] = 0
                masking_array[13] = 0
                masking_array[15] = 0

        #If decoration station 1  and 2 are full, do not decorate 17 - 24 
        if (self.bakery_state.decorating_station_1.current_recipe.name.value > 0 and self.bakery_state.decorating_station_1.time_remaining > 0) and (self.bakery_state.decorating_station_2.current_recipe.name.value > 0 and self.bakery_state.decorating_station_2.time_remaining > 0):
            masking_array[16] = 0
            masking_array[17] = 0
            masking_array[18] = 0
            masking_array[19] = 0
    ...
    def step(self,action) -> Dict[str, Any]: 
        action_string = self.map_action_to_statement(action)
        baker = 0
        time = self.bakery_state._sim_time - self.yield_time
        ...
        if action_string != "wait" and self.bakery_state.decorating_station_1.current_recipe != None:
        ...
        if action_string != "wait" and self.bakery_state.decorating_station_2.current_recipe != None:
        ...
        if action_string ==  "wait":
            self.env.run(until=self.yield_time + self.env.now)
            self.bakery_state._sim_time = self.bakery_state._sim_time - self.yield_time
   
            if self.bakery_state.mixer_1.current_recipe.name == 'none':
                self.bakery_state.mixer_1.time_remaining = 0
            else:
                self.bakery_state.mixer_1.time_remaining = self.bakery_state.mixer_1.set_remaining_time()

            if self.bakery_state.mixer_2.name == 'none':
                self.bakery_state.mixer_2.time_remaining = 0
            else:
                self.bakery_state.mixer_2.time_remaining = self.bakery_state.mixer_2.set_remaining_time()

            if self.bakery_state.oven_1.name == 'none':
                self.bakery_state.oven_1.time_remaining = 0
            else:
                self.bakery_state.oven_1.time_remaining = self.bakery_state.oven_1.set_remaining_time()

            if self.bakery_state.oven_2.name == 'none':
                self.bakery_state.oven_2.time_remaining = 0
            else:
                self.bakery_state.oven_2.time_remaining = self.bakery_state.oven_2.set_remaining_time()

            if self.bakery_state.oven_3.name == 'none':
                self.bakery_state.oven_3.time_remaining = 0
            else:
                self.bakery_state.oven_3.time_remaining = self.bakery_state.oven_2.set_remaining_time()

            if self.bakery_state.decorating_station_1.name == 'none':
                self.bakery_state.decorating_station_1.time_remaining = 0
            else:
                self.bakery_state.decorating_station_1.time_remaining = self.bakery_state.decorating_station_1.set_remaining_time()

            if self.bakery_state.decorating_station_2.name == 'none':
                self.bakery_state.decorating_station_2.time_remaining = 0
            else:
                self.bakery_state.decorating_station_2.time_remaining = self.bakery_state.decorating_station_2.set_remaining_time()

            if self.debug == True:
                print('yield_time =',self.yield_time)
            ...
            if self.debug == True:
                print('yield_time =',self.yield_time)
                print('env =', self.env.now)
                print(self.bakery_state.mixer_1.current_recipe.name)
