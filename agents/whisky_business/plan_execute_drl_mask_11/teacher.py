...
class BaseTeacher(Teacher):
    ...
    def compute_action_mask(self, transformed_obs, action):
        co = int(round(1.3 * float(transformed_obs["cookies_demand"]),0))
        cp = int(round(1.1 * float(transformed_obs["cupcake_demand"])),0)
        ck = int(round(1.05 * float(transformed_obs["cake_demand"])),0)
        mask = [1] * 25

        if float(transformed_obs["cookies_demand"]) >= co:
            #cookies
            for i in range(7,12):
                mask[i] = 0

        if float(transformed_obs["cupcake_demand"]) >= cp:
            for i in range(13,18):
                mask[i] = 0

        if float(transformed_obs["cake_demand"]) >= ck:
