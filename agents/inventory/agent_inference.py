...
def start():
    ...
    for i in range(30):
        ...
        action = action[0]
        action = [action*100 for i in action]
    ...
    print(f"Final Balance {df['balance'].to_list()[-1]}" )
    ...
    obs, info = sim.reset()
    for i in range(30):
        action = [random.randint(0,100),random.randint(0,100)]
        obs, reward, done, truncated, info = sim.step(action)

        df_temp = pd.DataFrame(columns=['inventory','balance','num_ordered','order_cutoff','order_target','time'],
        data=[[obs[0],obs[1],obs[2],action[0], action[0],i]])
        df = pd.concat([df, df_temp])
    ...
    plt.plot(df.reset_index()['time'],df.reset_index()['inventory'])
    plt.ylabel("Inventory level")
    plt.title(f"Final Balance (Random Agent) {round(df['balance'].to_list()[-1],1)} $" )
    ...
    print(f"Final Balance (Random Agent) {df['balance'].to_list()[-1]}" )


    plt.savefig(f"{PATH}/inference_figure_random_agent.png")
