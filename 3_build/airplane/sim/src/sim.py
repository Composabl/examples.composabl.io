...
class Env(gym.Env):
    ...
    def reset(self):
        ...
        B = np.array([[0.01, 1], [-0.18, -0.04], [-1.16, 0.598], [0, 0]])

        C = np.array([[1, 0, 0, 0], [0, -1, 0, 7.74]])
    ...
    def step(self, action):
        ...
        self.e1 = (
            0.001 if (-9.5 < self.y1 < -8.5) else abs(self.y1 - ((-9.5 + -8.5) / 2))
        )  # (-9.5,-8.5)
        self.e2 = (
            0.001 if (4.6 < self.y2 < 5.4) else abs(self.y2 - ((4.6 + 5.4) / 2))
        )  # (4.6,5.4)
